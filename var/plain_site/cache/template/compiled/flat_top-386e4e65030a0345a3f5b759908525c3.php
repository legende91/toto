<?php
// URI:       design/base/templates/menu/flat_top.tpl
// Filename:  design/base/templates/menu/flat_top.tpl
// Timestamp: 1342179292 (Fri Jul 13 13:34:52 CEST 2012)
$oldSetArray_39bbf96d44ad74afe88cbeed54c42a49 = isset( $setArray ) ? $setArray : array();
$setArray = array();
$tpl->Level++;
if ( $tpl->Level > 40 )
{
...
$text .= '<div id="topmenu">
    ...
    <h3 class="hide">Menu supérieur</h3>
...
$vars[$currentNamespace]['root_node'] = $var;
unset( $var );
unset( $var );
unset( $var1 );
unset( $var1 );
...
$var2 = compiledFetchAttribute( $var1, 'sort_array' );
unset( $var1 );
$var1 = $var2;
...
while ( is_object( $var ) and method_exists( $var, 'templateValue' ) )
    $var = $var->templateValue();
$vars[$currentNamespace]['menuitems'] = $var;
unset( $var );
$text .= '
    ';
// if begins
...
unset( $if_cond1 );
unset( $if_cond1 );
$if_cond1 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menuitems', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menuitems'] : null;
...
unset( $loopItem );
unset( $loopItem );
$loopItem = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menuitems', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menuitems'] : null;
...
$var4 = compiledFetchAttribute( $var3, 'path' );
unset( $var3 );
$var3 = $var4;
...
unset( $var4 );
unset( $var4 );
$var4 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menu', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menu'] : null;
...
while ( is_object( $var ) and method_exists( $var, 'templateValue' ) )
    $var = $var->templateValue();
$vars[$currentNamespace]['selected'] = $var;
unset( $var );
$text .= '            ';
// if begins
unset( $if_cond );
unset( $if_cond1 );
unset( $if_cond1 );
$if_cond1 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menu', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menu'] : null;
...
unset( $var1 );
unset( $var1 );
$var1 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'selected', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['selected'] : null;
if (! isset( $var1 ) ) $var1 = NULL;
while ( is_object( $var1 ) and method_exists( $var1, 'templateValue' ) )
    $var1 = $var1->templateValue();
while ( is_object( $var1 ) and method_exists( $var1, 'templateValue' ) )
...
unset( $if_cond2 );
unset( $if_cond2 );
$if_cond2 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menu', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menu'] : null;
$if_cond3 = compiledFetchAttribute( $if_cond2, 'index' );
unset( $if_cond2 );
$if_cond2 = $if_cond3;
...
unset( $if_cond2 );
unset( $if_cond2 );
$if_cond2 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'menuitems', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['menuitems'] : null;
if (! isset( $if_cond2 ) ) $if_cond2 = NULL;
while ( is_object( $if_cond2 ) and method_exists( $if_cond2, 'templateValue' ) )
    $if_cond2 = $if_cond2->templateValue();
...
$text .= '            ';
unset( $vars[$currentNamespace]['selected'] );
$text .= '    ';
...
$text .= '    ';
unset( $vars[$currentNamespace]['root_node'] );
unset( $vars[$currentNamespace]['menuitems'] );
...
$setArray = $oldSetArray_39bbf96d44ad74afe88cbeed54c42a49;
$tpl->Level--;
?>
